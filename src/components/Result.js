   ...
   questions.forEach(q => { q.isCorrect = q.options.every(x => x.selected === x.isAnswer ) })
        ...
        <div className="result">
            ...
            {questions.map((q, index) =>
                <div key={q.id} className={`mb-2 ${q.isCorrect ? count++ && 'bg-success' : 'bg-danger'}`}>
            ...
            <h4 className="alert alert-info text-center">Your Score is : {count }</h4>
            <h4 className="alert alert-info text-center">{ count >= 15 ? "  Great .." : "  Please Try again ..."  }</h4>
